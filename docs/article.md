...
Ну и естественно обработка ошибок: python-исключения, конвертация сишных ошибок в python-исключения.
...
Элемент канала сообщений инкапсулирует передаваемые данные и метаинформацию:
...
Особое внимание уделено двухфазному закрытию, fallback-сериализации и автоматическому разрешению ссылок. Все это вместе гарантирует безопасное завершение при параллельных операциях, обработку объектов вне стандартного `_PyXIData_t`-формата.
...
Операции защищенные (не путайте с атомарными), существуют таймауты блокировок для предотвращения взаимных блокировок (извиняюсь за тавтологию).
