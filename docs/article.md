...
- Python часть в виде PEP-734 был добавлена в 3.14 уже после feature freeze
- Изначально планировалось добавить его как модуль interpreters, однако в последний момент он стал concurrent.interpreters, вот тут доступно (https://discuss.python.org/t/decision-making-process-improvements-or-my-frustrations-with-how-pep-734-has-played-out/95985) большое обсуждение.

Модуль добавляет модуль interpreters (concurrent.interpreters). Это включает объекты Interpreter, представляющие базовые интерпретаторы. Модуль также предоставит базовый класс Queue для связи между интерпретаторами.
...
Без модуля stdlib пользователи ограничены API C , что ограничивает их возможности экспериментировать и использовать преимущества нескольких интерпретаторов.

Модуль будет включать базовый механизм для общения между переводчиками. Без него несколько переводчиков будут гораздо менее полезной функцией.
...
Интерпретаторы создаются через API C с помощью Py_NewInterpreterFromConfig()(или Py_NewInterpreter(), который является легкой оберткой вокруг Py_NewInterpreterFromConfig()). Эта функция делает следующее:
...
Некоторые функции мы опустим, если они малозначительные (по типу `_Py_GetMainfile`), вы их сможете просмотреть сами.
 ...
 + Функция `runpy_run_path` вызывает запуск `runpy` вместе с путем;
 + Функция `set_exc_with_cause` создает исключение с причиной.
...
4. **Управление сессиями:**
   - `_PyXI_session`: Контекст выполнения в другом интерпретаторе
...
Для межъядерного и многопроцессорного взаимодействуя идет реализация безопасного разделения буферов между интерпретаторами:
...
Для сереализации и десереализации сложных объектов есть механизм `_PyXIData`. Он используется в качестве аргумента к функциям работы со сложными объектами. А также есть поддержка разделяемых объектов через `shared` параметры.
...
Среди особенностей данного файла можно выделить безопасную работу с файлами: очистка состояний (`module_clear`, `traverse_module_state`), деаллокаторы (`xibufferview_dealloc`), и использование `Py_buffer` для работы с разделяемыми буферами.

Объекты интерпретаторов строго изолированы, существует переключение контекстов через `_PyXI_Enter` и `_PyXI_Exit`. Ошибки изоляции также обрабатывается через `unwrap_not_shareable`.
...
# А подробнее?
Процесс один, но интерпретаторов несколько и у каждого свой GIL. Все они делят одну выделенную память. Для защиты от перетерания данных разными интерпретаторами используется pickle, он не допускает мутации одной памяти из разных источников.
...
Также на PyCON US-24 презентовали функционал подинтепретатаров и free-threading. Видео-доклад можно посмотреть [здесь](https://youtu.be/Mp5wKOL4L2Q?si=vP-u9utukkzWpBkL).

Процессы, кстати, запускаются на одном ядре, но можно и на разных.

Потоки созданные под подинтерпретатором координируются только ОС, а потоки внутри каждого интерпретатора, координируются соответствующим GIL.
...
Теперь создание подинтепретаторов может дать буст к скорости благодаря отдельному GIL. И так плавно переходим к бенчмаркам.
...
Бенчмарк использует IO-bound и CPU-bound задачи. Он запускает простую версию, многопоточную с GIL и без GIL, через мультипроцессинг и сами подинтерпретаторы. 
...
В субинтерпретаторах каждый вызов `interp.exec()` требует сериализировать данные, переключение контекста между интерпретаторами, создание нового GIL для каждой операции. Само создание интерпретаторов дорогое, и поэтому их лучше выбирать для CPU-bound - ибо они дают истинный параллелизм на нескольких ядрах.
...
# Общие выводы
К просмотру рекомендаю [интервью CPython Core разработчика Никиты Соболева и разработчика модуля сабинтерпретаторов Эрика Сноу](https://www.youtube.com/watch?v=VBiaNNpLzWA).
...
3. Модули расширений: Требуют перехода на многофазную инициализацию (PEP 489) и heap-типы. Библиотеки вроде OpenSSL (через ssl модуль) — особый случай, где разделение состояния между интерпретаторами всё ещё проблематично.
...
Особенно перспективна интеграция с asyncio. Представьте: несколько изолированных эвент-лупов, работающих параллельно на разных ядрах. В питоне уже давно идет тенденция на ускорение - и весьма успешно достигается, пусть и с некоторыми оговорками.
...
Эрик скептичен насчёт массового использования субинтерпретаторов напрямую. В принципе, многие с ним согласятся, так как их ниша это библиотеки для высокоуровневых абстракций, веб-фреймворки, обработка данных. А также как альтернатива multiprocessing - ресурсы ОС экономятся лучше, коммуникация в рамках процесса быстрее.
